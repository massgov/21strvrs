...
DECLARE @TargetTable VARCHAR(128) = 'DeathPronouncement',  -- replace this 
...
DECLARE        
        ...
        @jiraID VARCHAR(16) = 'TBD', @author VARCHAR(128) = 'Rashmi Nagaraj', -- replace author name and jira #
        @printflag bit = 1 , 
		@seq INT = 0 ,
		...
		@stagingdb VARChar(128) = '[RVRS_Staging].' , --For PROD leave this blank else keep [RVRS_Staging].
        @LoadNotes VARCHAR(MAX) = ' + CASE WHEN LoadNote !='''' THEN ''||'' + LoadNote ELSE '''' END '
  DECLARE     @Workdb VARCHAR(128) = '[RVRS_testdb].',  -- For PROD leave this blank or set to @stagingdb else keep [RVRS_testdb].
		@rc INT = 0,
    ...
    DECLARE	@targetloc varchar(128) = @workdb -- For PROD replace this by [RVRS_PROD].[RVRS_ODS] or else keep @workdb. 
...
('PRO_DATE',1,'','','PRO_DATE', 'VARCHAR(128)', NULL,NULL),
('RIGHT(PRO_DATE,4)',1,'','','PronouncedYear', 'VARCHAR(128)',NULL,NULL ),
('LEFT(PRO_DATE,2)',1,'','','PronouncedMonth', 'VARCHAR(128)', NULL,NULL ),
('SUBSTRING(PRO_DATE,4,2)',1,'','','PronouncedDay', 'VARCHAR(128)', NULL,NULL ),
('PRO_TIME',1,'','','PRO_TIME', 'VARCHAR(128)', NULL,NULL),
('LEFT(PRO_TIME,2)',1,'','','PronouncedHour', 'VARCHAR(128)', NULL,NULL ),
('RIGHT(PRO_TIME,2)',1,'','','PronouncedMinute', 'VARCHAR(128)', NULL,NULL ),
('PRO_TIME_IN',1,'','','PronouncedTimeInd', 'VARCHAR(128)', NULL,'''NULL'''),
('FL_PRONOUNCEMENT_EXISTS',1,'','','FL_PRONOUNCEMENT_EXISTS', 'VARCHAR(128)', NULL,NULL),
...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
						...
						('PRO_DATE','ISDATE(REPLACE(REPLACE (COALESCE(PRO_DATE,''01/01/1900''),''/9999'', ''/1900''),''99/'',''01/''))=0','Not a valid Date of Pronouncement',  'Error'),
						('PRO_DATE','Try_Cast(PRO_DATE AS DateTime)>GETDATE() OR Try_Cast(PRO_DATE AS DateTime)< ''01/01/2014'' ','Date of Pronouncement is greater than Current Date Or less than 2014','Error'), 
						('PRO_DATE','PRO_DATE NOT LIKE ''[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]''','Date of Pronouncement is not in valid format','Error'),
						('PRO_DATE,DOD','TRY_CAST (DOD AS DateTime)<TRY_CAST(DOD AS DateTime)','Date of Pronouncement is greater than Date of Death','Error'),
												
						('PRO_TIME,DimPronouncedTimeIndId','((PronouncedTimeInd =''M'' AND ISDATE(REPLACE(PRO_TIME,''99'',''01'')) = 0) OR (PronouncedTimeInd IN( ''A'', ''P'') 
							AND ISDATE(PRO_TIME + REPLACE(REPLACE(PronouncedTimeInd,''A'',''AM''),''P'',''PM'')) = 0))','Time of Pronouncement not in a valid  range','Error'),
						('PRO_TIME','PRO_TIME NOT LIKE ''[0-9][0-9]:[0-9][0-9]''','Not a valid format for Time of Pronouncement','Error'),
						('PRO_TIME,DimPronouncedTimeIndId','PRO_TIME LIKE ''12:00'' AND PronouncedTimeInd NOT IN (''N'',''D'')', 'Time of Pronouncement not in align with Time Indicator', 'Error'),
						('FL_PRONOUNCEMENT_EXISTS,PRO_DATE,PRO_TIME','(FL_PRONOUNCEMENT_EXISTS = ''Y'' AND PRO_TIME IS  NULL ) OR (FL_PRONOUNCEMENT_EXISTS = ''Y'' AND PRO_DATE IS  NULL)', ' If FL_PRONOUNCEMENT_EXISTS is YES Pronouncement Date and Time should be blank', 'Warning'),
						('PRO_DATE,PRO_TIME','( PRO_TIME IS NOT NULL AND PRO_DATE IS  NULL ) ', ' Pronouncement Date is blank when Pronouncement Time is not blank', 'Error')
...
insert into  #dc  (SourceColumn,TableName,ColumName,DimColumn, ObjectType,DataType, DC, OtherFilter, DCType,DCTableName,DimFilter) values --DCType = OtherOnly, StandardOnly, Standard&Other
	('PronouncedTimeInd','DimTimeInd', 'DimPronouncedTimeIndId','Abbr', 'Dim', 'INT',0, NULL, NULL,NULL,NULL)
