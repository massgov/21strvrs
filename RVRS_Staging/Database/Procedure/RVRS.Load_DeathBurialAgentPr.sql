...
Jul 31 2023 		Sailendra Singh						RVRS 173 : LOAD DECEDENT DeathBurialAgent DATA FROM STAGING TO ODS

*****************************************************************************************
 For testing diff senarios you start using fresh data
*****************************************************************************************
DELETE FROM [RVRS_testdb].[RVRS].[DeathOriginal] WHERE Entity = 'DeathBurialAgent'
TRUNCATE TABLE [RVRS_testdb].[RVRS].[DeathBurialAgent]
DROP TABLE [RVRS_testdb].[RVRS].[DeathBurialAgent_Log]
DELETE FROM [RVRS_testdb].[RVRS].[Execution] WHERE Entity = 'DeathBurialAgent'
...
SET @LastLoadedDate=(SELECT MAX(LastLoadDate) FROM [RVRS_testdb].[RVRS].[Execution] WITH(NOLOCK) WHERE Entity='DeathBurialAgent' AND ExecutionStatus='Completed')
...
PRINT '2'  + CONVERT (VARCHAR(50),GETDATE(),109)
		        ...
		        SELECT    D.DEATH_REC_ID AS SrId
					  ,P.PersonId ,COALESCE(BURIAL_AGENT_GNAME,'NULL') FirstName,COALESCE(BURIAL_AGENT_MNAME,'NULL') MiddleName,COALESCE(BURIAL_AGENT_LNAME,'NULL') LastName,COALESCE(BURIAL_AGENT_SUFFIX,'NULL') Suffix,COALESCE(BURIAL_AGENT_TITLE,'NULL') BurialAgentTitle
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE DimBurialAgentTitleId WITH CORRECT VALUE,
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_Staging].[RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.BurialAgentTitle
...
--scenario 4
 ...
 SELECT * FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DimSuffix] DS WITH(NOLOCK) 
			 SELECT * FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DimBurialAgentTitle] DS WITH(NOLOCK) 
...
SET @LastLoadDate = (SELECT MAX(SrUpdatedDate) FROM #Tmp_HoldData)
			...
			INSERT INTO [RVRS_testdb].[RVRS].[DeathBurialAgent]
			(
...
PRINT ' Number of Record = ' +  CAST(@TotalLoadedRecord AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathBurialAgent]
/*
----------------------------------------------------------------------------------------------------------------------------------------------
11 - LOAD to Log    
----------------------------------------------------------------------------------------------------------------------------------------------
...
INSERT INTO [RVRS_testdb].[RVRS].[DeathBurialAgent_Log]
			(
...
PRINT ' Number of Record = ' +  CAST(@TotalErrorRecord AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathBurialAgent_Log] 
/*
----------------------------------------------------------------------------------------------------------------------------------------------
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR DimSuffixId*/
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_testdb].[RVRS].[DeathBurialAgent]  PA ON PA.PersonId=MT.PersonId 	
...
PRINT ' Number of Record = ' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathOriginal] WHERE Entity = 'DeathBurialAgent'
/*
----------------------------------------------------------------------------------------------------------------------------------------------
...
PRINT ' Number of Record = ' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
 select * from [RVRS_testdb].[RVRS].[Execution] WHERE Entity= 'DeathBurialAgent'
END TRY
 BEGIN CATCH
		PRINT 'CATCH'
		UPDATE [RVRS_testdb].[RVRS].[Execution]
