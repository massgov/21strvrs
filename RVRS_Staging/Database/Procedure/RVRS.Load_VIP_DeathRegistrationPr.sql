...
Jan 20 2023 		Sailendra Singh						RVRS 170 : LOAD DECEDENT DeathRegistration DATA FROM STAGING TO ODS

*****************************************************************************************
 For testing diff senarios you start using fresh data
*****************************************************************************************
DELETE FROM [RVRS_testdb].[RVRS].[DeathOriginal] WHERE Entity = 'DeathRegistration'
TRUNCATE TABLE [RVRS_testdb].[RVRS].[DeathRegistration]
DROP TABLE [RVRS_testdb].[RVRS].[DeathRegistration_Log]
DELETE FROM [RVRS_testdb].[RVRS].[Execution] WHERE Entity = 'DeathRegistration'
...
IF OBJECT_ID('[RVRS_testdb].[RVRS].[DeathRegistration_Log]') IS NULL 
	CREATE TABLE [RVRS_testdb].[RVRS].[DeathRegistration_Log] (Id BIGINT IDENTITY (1,1), SrId VARCHAR(64), RegistrationNumber_DC VARCHAR(128),DepositionNumber_DC VARCHAR(128),RegVolume_DC VARCHAR(128),RegPage_DC VARCHAR(128), [DimRegistrarTypeInternalId] INT,[PersonId] BIGINT,[RegistrationDate] VARCHAR(10),[DimRegistrarId] INT,[RegistrationNumber] VARCHAR(32),[AmendmentDate] VARCHAR(16),[DimRecordAccessId] INT,[RegVolume] VARCHAR(24),[RegPage] VARCHAR(10),[DepositionNumber] VARCHAR(24),[ArchivalPrintedDate] VARCHAR(10),[FlArchived] VARCHAR(16),[FlAcknowledge] VARCHAR(16),[PreviousVolumePage] VARCHAR(15),[DimPageEntryTypeId] INT,[FlRegistered] VARCHAR(16),[SrcRegistererUserId] INT,[FlSearchable] VARCHAR(16), RegistrarId VARCHAR(128),RegistrarName VARCHAR(128),PageEntryTypeId VARCHAR(128),RecordAccessId VARCHAR(128),OCCUR_REGIS_DATE VARCHAR(128),RESIDE_REGIS_DATE VARCHAR(128),ST_REGIS_DATE VARCHAR(128),DOD VARCHAR(128),VRV_REC_REPLACE_NBR VARCHAR(128),FL_OCCUR_IS_RESIDE VARCHAR(128),SrCreatedDate DATETIME,SrUpdatedDate DATETIME,CreatedDate DATETIME NOT NULL DEFAULT GetDate(),DeathRegistration_Log_Flag BIT ,LoadNote VARCHAR(MAX))
...
SET @LastLoadedDate=(SELECT MAX(LastLoadDate) FROM [RVRS_testdb].[RVRS].[Execution] WITH(NOLOCK) WHERE Entity='DeathRegistration' AND ExecutionStatus='Completed')
...
PRINT '2'  + CONVERT (VARCHAR(50),GETDATE(),109)
		        ...
		        SELECT    D.DEATH_REC_ID AS SrId
					  ,P.PersonId ,COALESCE(1000000 + OCCUR_REGIS_NAME,'0') RegistrarId,COALESCE(OCCUR_REGIS_NAMEL,'NULL') RegistrarName,OCCUR_REGIS_NUM RegistrationNumber,OCCUR_REGIS_DATE RegistrationDate,OCCUR_REGIS_VOLUME RegVolume,OCCUR_REGIS_PAGE RegPage,OCCUR_DEPOSITION_NUM DepositionNumber,OCCUR_AMENDMENT_DATE AmendmentDate,ARCHIVAL_COPY_PRINTED_DATE ArchivalPrintedDate,FL_OCCUR_ARCHIVED FlArchived,OCCUR_REGISTERED_FL FlRegistered,FL_SEARCHABLE FlSearchable,REGISTERER_ID SrcRegistererUserId,NULL FlAcknowledge,NULL PreviousVolumePage,COALESCE(NULL,'NULL') PageEntryTypeId,COALESCE(NULL,'NULL') RecordAccessId,OCCUR_REGIS_DATE OCCUR_REGIS_DATE,RESIDE_REGIS_DATE RESIDE_REGIS_DATE,ST_REGIS_DATE ST_REGIS_DATE,DOD DOD,VRV_REC_REPLACE_NBR VRV_REC_REPLACE_NBR,FL_OCCUR_IS_RESIDE FL_OCCUR_IS_RESIDE,1 DimRegistrarTypeInternalId
...
PRINT '6'  + CONVERT (VARCHAR(50),GETDATE(),109)	
		...
		SELECT *
	        ...
	        ,CASE WHEN Try_Cast(OCCUR_REGIS_DATE AS DateTime)>Try_Cast(RESIDE_REGIS_DATE AS DateTime) 
						OR Try_Cast(OCCUR_REGIS_DATE AS DateTime)>Try_Cast(ST_REGIS_DATE AS DateTime) THEN 'OCCUR_REGIS_DATE,RESIDE_REGIS_DATE,ST_REGIS_DATE|Error:Date of Occurance must be first' ELSE '' END AS LoadNote_4
	        ...
	        ,CASE WHEN Try_Cast(OCCUR_REGIS_DATE AS DateTime)>Try_Cast(RESIDE_REGIS_DATE AS DateTime) 
						OR Try_Cast(OCCUR_REGIS_DATE AS DateTime)>Try_Cast(ST_REGIS_DATE AS DateTime) THEN 'OCCUR_REGIS_DATE,RESIDE_REGIS_DATE,ST_REGIS_DATE|Error:Date of Occurance must be first' ELSE '' END AS LoadNote_9
	        ,CASE WHEN ISDATE(REPLACE(REPLACE (COALESCE(ArchivalPrintedDate,'01/01/1900'),'/9999', '/1900'),'99/','01/'))=0 THEN 'ArchivalPrintedDate|Error:Not a valid Archival Printed Date' ELSE '' END AS LoadNote_10
	        ,CASE WHEN Try_Cast(ArchivalPrintedDate AS DateTime)<Try_Cast(RegistrationDate AS DateTime) THEN 'ArchivalPrintedDate,RegistrationDate|Error:Archival Printed Date is before Date of Registration ' ELSE '' END AS LoadNote_11
	        ,CASE WHEN FlAcknowledge NOT IN ('N','Y',NULL) THEN 'FlAcknowledge|Error:Not a valid value for FlAcknowledge' ELSE '' END AS LoadNote_12
	        ,CASE WHEN FlArchived NOT IN ('N','Y',NULL) THEN 'FlArchived|Error:Not a valid value for FlArchived' ELSE '' END AS LoadNote_13
	        ,CASE WHEN ArchivalPrintedDate IS NOT NULL AND FlArchived<>'Y'  THEN 'FlArchived,ArchivalPrintedDate|Error:If Archival Printed Date is not blank then the value for FlArchived must be Y' ELSE '' END AS LoadNote_14
	        ,CASE WHEN FlRegistered = 'Y' AND OCCUR_REGIS_DATE IS NULL THEN 'FlRegistered,OCCUR_REGIS_DATE|Error:If FlRegistered value is Yes then there must be Occurance Registered Date' ELSE '' END AS LoadNote_15
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE DepositionNumber WITH CORRECT VALUE,
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_Staging].[RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.DepositionNumber
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE Volume WITH CORRECT VALUE,
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_Staging].[RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.RegVolume
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE Page WITH CORRECT VALUE,
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_Staging].[RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.RegPage
			WHERE  DC.TableName='DeathRegistration_Volume'
...
/*THIS CODE IS TO GET MATCH FROM DimPageEntryType TABLE AND UPDATE THE DimPageEntryTypeId WITH CORRECT VALUE*/
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_PROD].[RVRS_ODS].[RVRS].[DimPageEntryType] DS WITH(NOLOCK) ON DS.Abbr=MT.PageEntryTypeId
	       ...
	       /*UPDATING THE DeathRegistration_Log_Flag FOR ALL THE RECORDS FOR WHICH WE WILL NOT HAVE A MATCH IN  TABLE*/
			...
			UPDATE #Tmp_HoldData_Final 
					SET DeathRegistration_Log_Flag=1
					   , LoadNote = 'PageEntryTypeId|Pending Review:Not a valid PageEntryTypeId' + CASE WHEN LoadNote !='' THEN '||' + LoadNote ELSE '' END 
...
SET @LastLoadDate = (SELECT MAX(SrUpdatedDate) FROM #Tmp_HoldData)
			...
			INSERT INTO [RVRS_testdb].[RVRS].[DeathRegistration]
			(
...
INSERT INTO [RVRS_testdb].[RVRS].[DeathRegistration_Log]
			(
				 SrId, RegistrationNumber_DC ,DepositionNumber_DC ,RegVolume_DC ,RegPage_DC 
				 , [DimRegistrarTypeInternalId],[PersonId],[RegistrationDate],[DimRegistrarId],[RegistrationNumber],[AmendmentDate],[DimRecordAccessId],[RegVolume],[RegPage],[DepositionNumber],[ArchivalPrintedDate],[FlArchived],[FlAcknowledge],[PreviousVolumePage],[DimPageEntryTypeId],[FlRegistered],[SrcRegistererUserId],[FlSearchable]	
				 , RegistrarId,RegistrarName,PageEntryTypeId,RecordAccessId,OCCUR_REGIS_DATE,RESIDE_REGIS_DATE,ST_REGIS_DATE,DOD,VRV_REC_REPLACE_NBR,FL_OCCUR_IS_RESIDE
				,SrCreatedDate
				,SrUpdatedDate
				,CreatedDate
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR RegistrationNumber*/
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_testdb].[RVRS].[DeathRegistration]  PA ON PA.PersonId=MT.PersonId  AND PA.DimRegistrarTypeInternalId=MT.DimRegistrarTypeInternalId	
