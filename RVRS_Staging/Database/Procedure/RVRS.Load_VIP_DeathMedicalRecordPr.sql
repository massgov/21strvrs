...
Nov  7 2022 		Sailendra Singh						RVRS 170 : LOAD DECEDENT DeathMedicalRecord DATA FROM STAGING TO ODS

*****************************************************************************************
 For testing diff senarios you start using fresh data
*****************************************************************************************
DELETE FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathOriginal] WHERE Entity = 'DeathMedicalRecord'
TRUNCATE TABLE [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathMedicalRecord]
DROP TABLE [RVRS].[DeathMedicalRecord_Log]
DELETE FROM [RVRS].[Execution] WHERE Entity = 'DeathMedicalRecord'
...
SET @LastLoadedDate=(SELECT MAX(LastLoadDate) FROM [RVRS].[Execution] WITH(NOLOCK) WHERE Entity='DeathMedicalRecord' AND ExecutionStatus='Completed')
...
PRINT '6'  + CONVERT (VARCHAR(50),GETDATE(),109)	
		...
		SELECT *
	        ...
	        ,CASE WHEN COALESCE(ME_TOD,'')<>COALESCE(TOD,'') THEN 'ME_TOD,TOD|Warning:Time of Death Mismatch in Decedent and Autopsy page ' ELSE '' END AS LoadNote_21
	        ,CASE WHEN ((TimeInd ='M' AND ISDATE(REPLACE(ME_TOD,'99','01')) = 0) OR (TimeInd IN( 'A', 'P') 
							AND ISDATE(ME_TOD + REPLACE(REPLACE(TimeInd,'A','AM'),'P','PM')) = 0)) THEN 'ME_TOD,TimeInd|Error:Time of death not in a valid  range' ELSE '' END AS LoadNote_22
	        ,CASE WHEN ME_TOD NOT LIKE '[0-9][0-9]:[0-9][0-9]' THEN 'ME_TOD|Error:Not a valid format for Time of Death' ELSE '' END AS LoadNote_23
	        ,CASE WHEN ME_TOD LIKE '12:00' AND TimeInd NOT IN ('N','D') THEN 'ME_TOD,TimeInd|Error:Time of Death not in align with Time Indicator' ELSE '' END AS LoadNote_24
	        ...
	        ,CASE WHEN COALESCE(ME_PRO_TIME,'')<>COALESCE(ProTimeTab5A,'') THEN 'ME_PRO_TIME,PRO_TIME|Warning:Pronounced Time of Death in Pronouncement and Certfier Mismatch' ELSE '' END AS LoadNote_30
	        ,CASE WHEN ((PronouncedTimeInd ='M' AND ISDATE(REPLACE(ME_PRO_TIME,'99','01')) = 0) OR (PronouncedTimeInd IN( 'A', 'P') 
							AND ISDATE(ME_PRO_TIME + REPLACE(REPLACE(PronouncedTimeInd,'A','AM'),'P','PM')) = 0)) THEN 'ME_PRO_TIME,PronouncedTimeInd|Error:Time of Pronouncement is not in a valid range' ELSE '' END AS LoadNote_31
	        ...
	        ,CASE WHEN ME_PRO_TIME LIKE '12:00' AND PronouncedTimeInd NOT IN ('N','D') THEN 'ME_PRO_TIME,PronouncedTimeInd|Error:Pronounced Time of Death not in align with Pronounced Time Indicator' ELSE '' END AS LoadNote_33
...
*/
	...
	ALTER TABLE #Tmp_HoldData_Final ADD  FirstName_DC VARCHAR(128),MiddleName_DC VARCHAR(128),LastName_DC VARCHAR(128),RecordNumber_DC VARCHAR(128),CaseNumber_DC VARCHAR(128),Suffix_DC VARCHAR(128), FirstName_Flag BIT NOT NULL DEFAULT 0 ,MiddleName_Flag BIT NOT NULL DEFAULT 0 ,LastName_Flag BIT NOT NULL DEFAULT 0 ,RecordNumber_Flag BIT NOT NULL DEFAULT 0 ,CaseNumber_Flag BIT NOT NULL DEFAULT 0 ,Suffix_Flag BIT NOT NULL DEFAULT 0 ,DeathMedicalRecord_Log_Flag BIT NOT NULL DEFAULT 0,LoadNote VARCHAR(MAX) 
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE FirstName WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN DeathMedicalRecord_FirstName TABLE*/		
			...
			WHERE  DC.TableName='DeathMedicalRecord_FirstName'
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE MiddleName WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN DeathMedicalRecord_MiddleName TABLE*/		
			...
			WHERE  DC.TableName='DeathMedicalRecord_MiddleName'
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE LastName WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN DeathMedicalRecord_LastName TABLE*/		
			...
			WHERE  DC.TableName='DeathMedicalRecord_LastName'
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE RecordNumber WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN DeathMedicalRecord_RecordNumber TABLE*/		
			
  			
			UPDATE MT
				SET 
				RecordNumber_DC= DC.Mapping_Current
				,MT.RecordNumber_Flag=1
				,MT.LoadNote='RecordNumber|Warning:RecordNumber got value from data conversion' + CASE WHEN LoadNote !='' THEN '||' + LoadNote ELSE '' END  
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.RecordNumber
			WHERE  DC.TableName='DeathMedicalRecord_RecordNumber'
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE CaseNumber WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN DeathMedicalRecord_CaseNumber TABLE*/		
			
  			
			UPDATE MT
				SET 
				CaseNumber_DC= DC.Mapping_Current
				,MT.CaseNumber_Flag=1
				,MT.LoadNote='CaseNumber|Warning:CaseNumber got value from data conversion' + CASE WHEN LoadNote !='' THEN '||' + LoadNote ELSE '' END  
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS].[Data_Conversion] DC WITH(NOLOCK) ON DC.Mapping_Previous=MT.CaseNumber
			WHERE  DC.TableName='DeathMedicalRecord_CaseNumber'
...
/*THIS CODE IS TO GET MATCH FROM [RVRS].[Data_Conversion] TABLE AND UPDATE THE DimSuffixId WITH CORRECT VALUE,
	       ...
	       /*UPDATING THE DeathMedicalRecord_Log_Flag FOR ALL THE RECORDS FOR WHICH WE WILL NOT HAVE A MATCH IN [RVRS].[Data_Conversion] TABLE*/
...
SET @LastLoadDate = (SELECT MAX(SrUpdatedDate) FROM #Tmp_HoldData)
			...
			INSERT INTO [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathMedicalRecord]
			(
...
INSERT INTO [RVRS].[DeathMedicalRecord_Log]
			(
				 SrId, FirstName_DC ,MiddleName_DC ,LastName_DC ,RecordNumber_DC ,CaseNumber_DC ,Suffix_DC 
				 , [PersonId],[RecordNumber],[CaseYear],[CaseNumber],[FirstName],[MiddleName],[LastName],[DimSuffixId],[DimSexId],[BirthYear],[BirthMonth],[BirthDay],[DeathYear],[DeathMonth],[DeathDay],[DeathHour],[DeathMinute],[DimTimeIndId],[PronouncedYear],[PronouncedMonth],[PronouncedDay],[PronouncedHour],[PronouncedMinute],[DimPronouncedTimeIndId]	
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR FirstName*/
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathMedicalRecord]  PA ON PA.PersonId=MT.PersonId 	
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR RecordNumber*/
			...
			SELECT MT.SrId AS SrId
				...
				,'DeathMedicalRecordId' AS EntityColumnName
				,PA.DeathMedicalRecordId AS EntityId
				,'RecordNumber' AS ConvertedColumn
				,MT.RecordNumber AS OriginalValue
				,MT.RecordNumber_DC AS ConvertedValue
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathMedicalRecord]  PA ON PA.PersonId=MT.PersonId 	
			WHERE MT.RecordNumber_Flag=1
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR CaseNumber*/
			...
			SELECT MT.SrId AS SrId
				...
				,'DeathMedicalRecordId' AS EntityColumnName
				,PA.DeathMedicalRecordId AS EntityId
				,'CaseNumber' AS ConvertedColumn
				,MT.CaseNumber AS OriginalValue
				,MT.CaseNumber_DC AS ConvertedValue
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_PROD].[RVRS_ODS].[RVRS].[DeathMedicalRecord]  PA ON PA.PersonId=MT.PersonId 	
			WHERE MT.CaseNumber_Flag=1
