...
BEGIN
	...
	BEGIN TRY
		...
		SELECT D.DEATH_REC_ID AS SrId
			  ,P.PersonId
			  ,ISNULL(TRX_FLG,'0') AS LOOKUP_TRX_FLG
			  ,TRX_CAUSE_MANUAL AS CauseManual
			  ,ISNULL(TRX_INJRY_PLACE,'') AS InjuryPlace
			  ...
			  ,ISNULL(TRX_REC_AXIS_CD,'') AS RecordAxisCode
...
PRINT @TotalProcessedRecords
		...
		PRINT '3'
			SELECT HD.SrId AS SrId
			  ...
			  ,CASE WHEN HD.UnderlyingCause IS NOT NULL AND HD.CAUSE_CATEGORY1 IS NULL AND HD.CAUSE_CATEGORY2 IS NULL AND HD.CAUSE_CATEGORY3 IS NULL AND HD.CAUSE_CATEGORY4 IS NULL AND HD.CAUSE_CATEGORY5 IS NULL
					AND HD.CAUSE_CATEGORY6 IS NULL AND HD.CAUSE_CATEGORY7 IS NULL AND HD.CAUSE_CATEGORY8 IS NULL AND HD.CAUSE_CATEGORY9 IS NULL AND HD.CAUSE_CATEGORY10 IS NULL AND HD.CAUSE_CATEGORY11 IS NULL
					AND HD.CAUSE_CATEGORY12 IS NULL AND HD.CAUSE_CATEGORY13 IS NULL AND HD.CAUSE_CATEGORY14 IS NULL AND HD.CAUSE_CATEGORY15 IS NULL AND HD.CAUSE_CATEGORY16 IS NULL AND HD.CAUSE_CATEGORY17 IS NULL
					AND HD.CAUSE_CATEGORY18 IS NULL AND HD.CAUSE_CATEGORY19 IS NULL AND HD.CAUSE_CATEGORY20 IS NULL
					THEN 'TRX_CAUSE_ACME,CAUES_CATEGORY|Warning: TRX_CAUSE_ACME has value but CAUES_CATEGORY does not have value'
