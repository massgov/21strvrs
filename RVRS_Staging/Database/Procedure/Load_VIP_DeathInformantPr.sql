...
Oct 14 2022 		Foyzur Rahman						RVRS 169 : LOAD DECEDENT DeathInformant DATA FROM STAGING TO ODS

*****************************************************************************************
 For testing diff senarios you start using fresh data
*****************************************************************************************
DELETE FROM [RVRS_testdb].[RVRS].[DeathOriginal] WHERE Entity = 'DeathInformant'
TRUNCATE TABLE [RVRS_testdb].[RVRS].[DeathInformant]
DROP TABLE [RVRS_testdb].[RVRS].[DeathInformant_Log]
DELETE FROM [RVRS_testdb].[RVRS].[Execution] WHERE Entity = 'DeathInformant'
...
SET @LastLoadedDate=(SELECT MAX(LastLoadDate) FROM [RVRS_testdb].[RVRS].[Execution] WITH(NOLOCK) WHERE Entity='DeathInformant' AND ExecutionStatus='Completed')
...
PRINT '2'  + CONVERT (VARCHAR(50),GETDATE(),109)
		        ...
		        SELECT TOP 10000000  D.DEATH_REC_ID AS SrId
					  ,P.PersonId ,INFO_NME FirstName,INFO_LST_NME LastName,INFO_MIDD_NME MiddleName,ISNULL(INFO_SUFFIX,'NULL') Suffix,INFO_RELATION_LIST RelationList,ISNULL(INFO_RELATION,'NULL') InformantRelation,ISNULL(INFO_RELATION_OTHER,'NULL') OtherInformantRelation
...
*/
	...
	UPDATE #Tmp_HoldData_Final SET LoadNote =  IIF( LoadNote_1 <> '',  '||' + LoadNote_1, '') +  IIF( LoadNote_2 <> '',  '||' + LoadNote_2, '') +  IIF( LoadNote_3 <> '',  '||' + LoadNote_3, '') +  IIF( LoadNote_4 <> '',  '||' + LoadNote_4, '')
...
--scenario 4
 ...
 SELECT * FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DimSuffix] DS WITH(NOLOCK) 
			 SELECT * FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DimInformantRelation] DS WITH(NOLOCK) 
			 SELECT * FROM [RVRS_PROD].[RVRS_ODS].[RVRS].[DimOtherInformantRelation] DS WITH(NOLOCK) 
...
SET @LastLoadDate = (SELECT MAX(SrUpdatedDate) FROM #Tmp_HoldData)
			...
			INSERT INTO [RVRS_testdb].[RVRS].[DeathInformant]
			(
...
PRINT ' Number of Record = ' +  CAST(@TotalLoadedRecord AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathInformant]
/*
----------------------------------------------------------------------------------------------------------------------------------------------
11 - LOAD to Log    
----------------------------------------------------------------------------------------------------------------------------------------------
...
INSERT INTO [RVRS_testdb].[RVRS].[DeathInformant_Log]
			(
...
PRINT ' Number of Record = ' +  CAST(@TotalErrorRecord AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathInformant_Log] 
/*
----------------------------------------------------------------------------------------------------------------------------------------------
...
/*INSERTING DATA INTO RVRS.DeathOriginal FOR THE RECORDS WHERE WE HAVE A CONVERSION FOR DimSuffixId*/
			...
			FROM #Tmp_HoldData_Final MT
			JOIN [RVRS_testdb].[RVRS].[DeathInformant]  PA ON PA.PersonId=MT.PersonId 	
...
PRINT ' Number of Record = ' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
 ...
 select * from [RVRS_testdb].[RVRS].[DeathOriginal] WHERE Entity = 'DeathInformant'
/*
----------------------------------------------------------------------------------------------------------------------------------------------
...
PRINT ' Number of Record = ' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
 select * from [RVRS_testdb].[RVRS].[Execution] WHERE Entity= 'DeathInformant'
END TRY
 BEGIN CATCH
		PRINT 'CATCH'
		UPDATE [RVRS_testdb].[RVRS].[Execution]
