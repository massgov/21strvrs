...
DECLARE @TargetTable VARCHAR(128) = 'Occupation',  -- replace this 
...
DECLARE        
        ...
        @jiraID VARCHAR(16) = '163', @author VARCHAR(128) = 'Foyzur Rahman', -- replace author name and jira #
        @printflag bit = 1 , 
		@seq INT = 0 ,
		...
		@stagingdb VARChar(128) = '' , --For PROD leave this blank 
  ...
  DECLARE     @Workdb VARCHAR(128) = '',  -- For PROD leave this blank or set to @stagingdb
    ...
    DECLARE	@targetloc varchar(128) = '[RVRS_PROD].[RVRS_ODS].'  --for staging test @Workdb --for prod '[RVRS_PROD].[RVRS_ODS].' For PROD replace this by [RVRS_PROD].[RVRS_ODS]. 
...
('OCCUP', 1,'','','Occupation','VARCHAR(128)', NULL,NULL),
('INDUST', 1,'','','Industry','VARCHAR(128)', NULL,NULL),
('Census_Occ_code', 1,'','','OccupCode','VARCHAR(128)', NULL,'''-2'''),
('Census_Ind_code', 1,'','','IndusCode','VARCHAR(128)', NULL,'''-2''')
...
/*INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
						...
						('FindingAvailable,AutopsyPerformed','FindingAvailable = ''Y'' AND AutopsyPerformed in (''N'',''U'')','FindingAvailable is Yes and Autopsy perfomed is No or Unknown','Error')
...
insert into  #dc  (SourceColumn,TableName,ColumName,DimColumn, ObjectType,DataType, DC, OtherFilter, DCType) values --DCType = OtherOnly, StandardOnly, Standard&Other
    ('Occupation','Occupation_Occupation', 'Occupation',NULL, 'Fact', 'Varchar(128)',1,NULL,NULL),
	('Industry','Occupation_Industry', 'Industry',NULL, 'Fact', 'Varchar(128)',1,NULL,NULL),
	('OccupCode','DimNioshOccupation', 'DimNioshOccupationId','Code', 'Dim', 'Varchar(128)',0,NULL,NULL),
	('IndusCode','DimNioshIndustry', 'DimNioshIndustryId','Code', 'Dim', 'Varchar(128)',0,NULL,NULL)   
...
'BEGIN TRY
	...
	BEGIN
		...
		PRINT   '
				...
				LEFT JOIN '+@stagingdb+'[RVRS].[Data_Conversion_Niosh_Occupation] C ON C.Industry=D.Indust AND C.Occupation=D.Occup				
...
*/
	...
	print '/*'
	print @SectionBreak 
	print '6 - Add/Update Flag on #Tmp_HoldData'
	print @SectionBreak
	print '*/
	'

    PRINT ' 
	ALTER TABLE #Tmp_HoldData ADD ' + IIF (@DcCreate IS NOT NULL , @DcCreate + ',', '') + IIF (@DcFlagCreate IS NOT NULL, @DcFlagCreate +',','')+  @TableFlag +' BIT NOT NULL DEFAULT 0,LoadNote VARCHAR(MAX) 

	 UPDATE #Tmp_HoldData SET LoadNote = ''''
	
	UPDATE #Tmp_HoldData SET ' + @TableFlag + ' = 0

	UPDATE #Tmp_HoldData SET ' + @TableFlag+ '= 1
	...
	BEGIN 
		...
		IF (@ObjectType = 'Dim')
			...
			ALTER TABLE #Tmp_HoldData ADD ' + @ColumnName + ' ' + @DataType  + '
	...
	--		FROM #Tmp_HoldData f WITH(NOLOCK)
 ...
 --          		SET @RecordCountDebug=@@ROWCOUNT 
   ...
   PRINT	'                '	+ @print + ''''  +   @recordNumberPrint + ''' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
	...
	--PRINT 
	--			'--scenario 4
	--			IF EXISTS(SELECT 1 FROM #Tmp_HoldData WHERE PersonId IS NULL 
...
'
  ...
  if @testflag = 1
	...
	'   SET @TotalPendingReviewRecord=(SELECT COUNT(1) FROM #Tmp_HoldData WHERE '+@TableFlag+ '=1
