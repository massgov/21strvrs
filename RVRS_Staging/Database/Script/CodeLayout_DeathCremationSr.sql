/*
  ...
  Date : 07/18/2023
  Description: This script will be use to generate code layout for Prod version
  ...
  Note: Added filter in where statement on line 22
*/

SET NOCOUNT ON
USE RVRS_STAGING

DECLARE @TargetTable VARCHAR(128) = 'DeathCremation',  -- replace this 
...
('CREM_CLEAR_ME',1,'','','MeCleared', 'VARCHAR(128)',NULL,'''NULL'''),
('CREM_CLEAR_COMMENTS',1,'','','ClearedComment', 'VARCHAR(128)',NULL,NULL),
('CREM_RELEASE_ME',1,'','','MeReleased', 'VARCHAR(128)',NULL,'''NULL'''),
('CREM_RELEASE_DATE_ME',1,'','','MeCremationDate', 'VARCHAR(128)',NULL,NULL),
('DOD_4_FD',1,'','','DOD_4_FD', 'VARCHAR(128)',NULL,NULL)
...
--If  FNRL_SERVICE_OOS = Y and RESPONSIBE_NAME= FNRL_NME, log pending for review and keep ASIDE 
--If the FNRL_SERVICE_OOS is N, RESPONSIBE_NAME<> FNRL_NME,  log Pending for Review and Keep it ASIDE
--If TRADE_FH_UNLISTED =  'N' OR FL_FUNERAL_HOME_UNLISTED = 'N' and  FNRL_NME <> FH NAME In VT_Funeral_Home ,  log warning and save ASIS 

						('DimMeClearedId,ClearedComment','ClearedComment IS NOT NULL AND MeCleared !=''Y''','Value in Cleared Comment Section and ME Cleared flag are not aligned',  'Error'),
						('DimMeClearedId,DimMeReleasedId','MeCleared = ''N'' AND MeReleased != ''N''','ME Released Flag and ME Cleared flag are not aligned',  'Error'),	
						
						('MeCremationDate','ISDATE(REPLACE(REPLACE (COALESCE(MeCremationDate,''01/01/1900''),''/9999'', ''/1900''),''99/'',''01/''))=0','Not a valid Date of Cremation',  'Error'),
						('MeCremationDate,DOD_4_FD ','TRY_CAST (MeCremationDate AS DateTime)<TRY_CAST(DOD_4_FD AS DateTime)','Date of Death is greater than Date of Cremation','Error'),
						('MeCremationDate','TRY_CAST (MeCremationDate AS DateTime)<''01/01/2014''','Date of Cremation is not in valid range','Error'),
						('MeCremationDate','MeCremationDate NOT LIKE ''[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]''','Date of Cremation is not in valid format','Error'),
						('MeCremationDate,DimMeClearedId','MeCremationDate IS NOT NULL AND MeCleared !=''Y''','ME Cremation Date and ME Cleared flag are not aligned',  'Error')	

						--('DISP_DATE','ISDATE(REPLACE(REPLACE (COALESCE(DISP_DATE,''01/01/1900''),''/9999'', ''/1900''),''99/'',''01/''))=0','Not a valid Date of Disposition',  'Error'),
						--('DISP_DATE,DOD_4_FD ','TRY_CAST (DISP_DATE AS DateTime)<TRY_CAST(DOD_4_FD AS DateTime)','Date of Death is greater than Date of Disposition','Error'),
						--('DISP_DATE','TRY_CAST (DISP_DATE AS DateTime)<''01/01/2014'' OR Try_Cast(DISP_DATE AS DateTime)>GETDATE()','Date of Dispotition is not in valid range','Error'),--this might be changed to 'or'
						--('DISP_DATE','DISP_DATE NOT LIKE ''[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]''','Date of Disposition is not in valid format','Error'),
						--('CREM_CEM_UNLISTED,DimDispPlaceId,VIP_VT_CEMETERIES_CD.CEM_NAME','CREM_CEM_UNLISTED = ''N'' AND DispPlace<>CEM_NAME','The Name of the Cemetery does not match the Name from Code table',  'Warning')
...
CREATE TABLE #dc (id INT identity(1,1), SourceColumn VARCHAR(128), TableName VARCHAR(128),  ColumName VARCHAR(128),DimColumn VARCHAR(128),Flag Bit, ObjectType VARCHAR(8) default 'Dim', DataType VARCHAR(128), DC Bit, OtherFilter VARCHAR(MAX), DCType VARCHAR(16), DCTableName VARCHAR(128), DimFilter VARCHAR(512) )
				/*If you have any columns that requires Dim ID then it should included in this Data Conversion Section*/

				--coming from source,Table in Data Conversion,Name of Column in Target,Dim Column to join source column
insert into  #dc  (SourceColumn,TableName,ColumName,DimColumn, ObjectType,DataType, DC, OtherFilter, DCType,DCTableName,DimFilter) values --DCType = OtherOnly, StandardOnly, Standard&Other
	...
	('MeCleared','DimYesNo', 'DimMeClearedId','Abbr', 'Dim', 'INT',0, NULL, NULL,NULL,NULL),
	('MeReleased','DimYesNo', 'DimMeReleasedId','Abbr', 'Dim', 'INT',0, NULL, NULL,NULL,NULL) 

	--('DispMethod','DimDispMethod', 'DimDispMethodId','Abbr', 'Dim', 'INT',1, ' AND MT.OtherDispMethod = DC.FilterValue AND DC.Mapping_Previous = ''O''', 'OtherOnly','[RVRS].[Data_Conversion]',NULL),
	--('OtherDispMethod','DimOtherDispMethod', 'DimOtherDispMethodId','OtherDispMethodDesc', 'Dim', 'INT',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL), --check name of column in fact and DC are in align
	--('DispPlace','DimDispPlace', 'DimDispPlaceId','DispPlaceDesc', 'Dim', 'INT',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL) 
	
	--('FuneralHomeName','DimFuneralHomeName', 'DimFuneralHomeNameId','FuneralHomeNameDesc', 'Dim', 'INT',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL)
