...
DECLARE @TargetTable VARCHAR(128) = 'DeathBurialAgent',  -- replace this 
...
DECLARE        
        ...
        @jiraID VARCHAR(16) = '173', @author VARCHAR(128) = 'Sailendra Singh', -- replace author name and jira #
        @printflag bit = 1 , 
		@seq INT = 0 ,
...
('BURIAL_AGENT_GNAME',1,'','','FirstName', 'VARCHAR(128)',NULL,NULL),
('BURIAL_AGENT_MNAME',1,'','','MiddleName', 'VARCHAR(128)',NULL,NULL),
('BURIAL_AGENT_LNAME',1,'','','LastName', 'VARCHAR(128)',NULL,NULL),
('BURIAL_AGENT_SUFFIX',1,'','','Suffix', 'VARCHAR(128)',NULL,'''NULL'''),
('BURIAL_AGENT_TITLE',1,'','','BurialAgentTitle', 'VARCHAR(128)',NULL,'''NULL''')
...
--If TRADE_FH_UNLISTED =  'N' OR FL_FUNERAL_HOME_UNLISTED = 'N' and  FNRL_NME <> FH NAME In VT_Funeral_Home ,  log warning and save ASIS 
						...
						('FirstName','FirstName IS NULL','FirstName is missing',  'Error'),
						('MiddleName','LEFT (Replace (MiddleName,''-'',''''),1) LIKE ''[^a-zA-Z]'' AND MiddleName NOT IN (''.'')','MiddleName does not start with Alphabet',  'Error'),
						('LastName','LEFT(LastName,1) LIKE ''[^a-zA-Z]'' and LastName NOT LIKE ''-%''','LastName does not start with Alphabet',  'Error'),	
						('LastName','LastName IS NULL','LastName is missing',  'Error')
...
('Suffix','DimSuffix', 'DimSuffixId','SuffixDesc', 'Dim', 'INT',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL),
('BurialAgentTitle','DimBurialAgentTitle', 'DimBurialAgentTitleId','BurialAgentTitleDesc', 'Dim', 'INT',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL)
