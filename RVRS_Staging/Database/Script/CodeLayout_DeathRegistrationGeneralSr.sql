...
DECLARE @TargetTable VARCHAR(128) = 'DeathRegistrationGeneral',  -- replace this 
...
('RECORD_REGIS_DATE',1,'','','RegistrationDate', 'VARCHAR(128)',NULL,NULL),
('RECORD_REGISTRAR',1,'','','RegistrarName', 'VARCHAR(128)',NULL,'''NULL'''),
('RECORD_REGIS_NUM',1,'','','RegistrationNumber', 'VARCHAR(128)',NULL,NULL),
('DATE_OF_AMENDMENT',1,'','','AmendmentDate', 'VARCHAR(128)',NULL,NULL),
('IND_ACCESS_STATUS',1,'','','RecordAccess', 'VARCHAR(128)',NULL,'''NULL'''),
('FL_UPDATE_PENDING',1,'','','FlUpdatePending', 'VARCHAR(128)',NULL,NULL),
('AMEND_IN_PROCESS',1,'','','FlAmendmentInProcess', 'VARCHAR(128)',NULL,NULL),
('VRV_REGISTERED_FLAG',1,'','','FlRegistered', 'VARCHAR(128)',NULL,NULL),
('IND_RECORD_OWNER',1,'','','RecordOwner', 'VARCHAR(128)',NULL,NULL),
('RECORD_REGISTRAR_ID',1,'','','ScrRegistererUserId', 'VARCHAR(128)',NULL,NULL),
('FL_DELAYED',1,'','','FlDelayed', 'VARCHAR(128)',NULL,NULL),
('FL_AMENDED',1,'','','FlAmended', 'VARCHAR(128)',NULL,NULL),
('IND_REGIS_STATUS',1,'','','RegistrationStatus', 'VARCHAR(128)',NULL,'''NULL'''),
('VRV_REC_REPLACE_NBR',1,'','','VRV_REC_REPLACE_NBR', 'VARCHAR(128)',NULL,NULL),
('OCCUR_REGIS_DATE',1,'','','OCCUR_REGIS_DATE', 'VARCHAR(128)',NULL,NULL),
('OCCUR_REGIS_NAMEL',1,'','','OCCUR_REGIS_NAMEL', 'VARCHAR(128)',NULL,NULL),
('OCCUR_REGIS_NUM',1,'','','OCCUR_REGIS_NUM', 'VARCHAR(128)',NULL,NULL),
('DOD',1,'','','DOD', 'VARCHAR(128)',NULL,NULL),
('ST_REGIS_DATE',1,'','','ST_REGIS_DATE', 'VARCHAR(128)',NULL,NULL)
...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
						...
						('RegistrationDate,VRV_REC_REPLACE_NBR,OCCUR_REGIS_DATE','VRV_REC_REPLACE_NBR = 0 AND OCCUR_REGIS_DATE<>RegistrationDate','The Occurance Date and Registration date must be same for Version 0 ','Error'),

						('RegistrarName,OCCUR_REGIS_NAMEL','VRV_REC_REPLACE_NBR = 0 AND RegistrarName<>OCCUR_REGIS_NAMEL','The Occurance Registrar Name and Registration date must be same for Version 0','Error'), 

						('RegistrationNumber,OCCUR_REGIS_NUM','VRV_REC_REPLACE_NBR = 0 AND RegistrationNumber<>OCCUR_REGIS_NUM','The Registration Number and Occurance Registrar Number must be same for Version 0 Record','Error'),
						('RegistrationNumber','RegistrationNumber IS NULL','There must be Registration Number for Registered records','Error'),
						...
						('FlUpdatePending','FlUpdatePending NOT IN (''N'',''Y'')','Not a valid value for FlUpdatePending','Error'), 

						('FlAmendmentInProcess','FlAmendmentInProcess NOT IN (''N'',''Y'')','Not a valid value for FlAmendmentInProcess','Error'), 
						('FlAmendmentInProcess VRV_REC_REPLACE_NBR','FlAmendmentInProcess = ''Y'' AND VRV_REC_REPLACE_NBR = 0','Not a valid value for FlAmendmentInProcess with respect to Amendment Version','Error'), 

						('FlRegistered','FlRegistered <> 1','Not a valid value for FlRegistered','Error'),
						('FlRegistered,OCCUR_REGIS_DATE','OCCUR_REGIS_DATE IS NULL AND FlRegistered = 1 ','When a record is registered then the Occurance information must be entered as well ','Error'),

						('RecordOwner','RecordOwner IS NULL','Record Owner should not be blank for registered records','Error'),
						('RecordOwner','ISNULL(RecordOwner,''RV'') NOT IN (''RV'',''OC'')','Not a valid value for Record Owner','Error'), 
						('RecordOwner,OCCUR_REGIS_DATE,RegistrationDate','RecordOwner = ''OC'' AND OCCUR_REGIS_DATE<>RegistrationDate','If the Record Owner is Occurance then Registration Date should match with Occurance Date','Error'), 
						('RecordOwner,ST_REGIS_DATE,RegistrationDate','RecordOwner = ''RV'' AND ST_REGIS_DATE<>RegistrationDate','If the Record Owner is the State then Registration Date should match with State Registration Date','Error'),
						

						('ScrRegistererUserId','ScrRegistererUserId IS NULL','ScrRegistererUserId should not be blank for registered records','Error'),

						('FlDelayed','ISNULL(FlDelayed,''Y'') NOT IN (''N'',''Y'')','Not a valid value for FlUpdatePending','Error'), 

						('FlAmended','ISNULL(FlAmended,1) NOT IN (0,1)','Not a valid value for FlAmended','Error'), 
						('FlAmended,VRV_REC_REPLACE_NBR','(FlAmended= 1 AND VRV_REC_REPLACE_NBR = 0)','Not a valid value for FlAmended for its version','Error') 
					
						--check validation for RegistrationStatusId
...
insert into  #dc  (SourceColumn,TableName,ColumName,DimColumn, ObjectType,DataType, DC, OtherFilter, DCType,DCTableName,DimFilter) values --DCType = OtherOnly, StandardOnly, Standard&Other
	...
	('RegistrationNumber','DeathRegistrationGeneral_RegistrationNumber', 'RegistrationNumber','', 'Fact', 'VARCHAR(128)',1, NULL, NULL,'[RVRS].[Data_Conversion]',NULL),
	('RegistrationStatus','DimRegistrationStatus', 'DimRegistrationStatusId','RegistrationStatusDesc', 'Dim', 'INT',0, NULL, NULL,NULL,NULL),
