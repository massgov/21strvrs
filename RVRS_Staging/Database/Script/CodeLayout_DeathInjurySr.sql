...
DECLARE @TargetTable VARCHAR(128) = 'DeathInjury',  -- replace this 
...
('DOD',1,'','','DOD', 'VARCHAR(128)', NULL,NULL),
...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
						...
						('DOI','ISDATE(REPLACE(REPLACE (COALESCE(DOI,''01/01/1900''),''/9999'', ''/1900''),''99/'',''01/''))=0','Not a valid Date of Injury',  'Error'),
						('DOI','Try_Cast(DOI AS DateTime)>GETDATE()','Date of Injury is greater than Current Date','Error'), 
						('DOI','DOI NOT LIKE ''[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]''','Date of Injury is not in valid format','Error'),
						('DOI,DOD','TRY_CAST (DOD AS DateTime)<TRY_CAST(DOI AS DateTime)','Date of Injury is greater than Date of Death','Error'),
						('DOI,Death.DimCertifierDesignId','DOI IS NOT NULL AND Certifier <>''MEDICAL EXAMINER'' AND DOI<>''99/99/9999''','When Date of Injury is populated Certifier should be Medical Examiner','Error'),
						('DOI,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND DOI IS NULL','When Manner of Death is Accident or Homicide or Suicide then Date of Injury should be populated','Error'),

						('TOI,DimInjuryTimeIndId','((InjuryTimeInd =''M'' AND ISDATE(REPLACE(TOI,''99'',''01'')) = 0) OR (InjuryTimeInd IN( ''A'', ''P'') 
							AND ISDATE(TOI + REPLACE(REPLACE(InjuryTimeInd,''A'',''AM''),''P'',''PM'')) = 0))','Time of Injury not in a valid  range','Error'),
						('TOI','TOI NOT LIKE ''[0-9][0-9]:[0-9][0-9]''','Not a valid format for Time of Injury','Error'),
						('TOI,Death.DimCertifierDesignId','TOI IS NOT NULL AND Certifier <>''MEDICAL EXAMINER'' AND TOI<>''99:99''','When Time of Injury is populated Certifier should be Medical Examiner','Error'),
						('TOI,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND TOI IS NULL','When Manner of Death is Accident or Homicide or Suicide then Time of Injury should be populated','Error'),

						('TOI,DimInjuryTimeIndId','TOI LIKE ''12:00'' AND InjuryTimeInd NOT IN (''N'',''D'')', 'Time of Injury not in align with Time Indicator', 'Error'),
						('DimInjuryTimeIndId,Death.DimCertifierDesignId','InjuryTimeInd  <> ''NULL''  AND Certifier <>''MEDICAL EXAMINER''','When Time Indicator of Injury is populated Certifier should be Medical Examiner','Error'),
						('DimInjuryTimeIndId,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND InjuryTimeInd = ''NULL''','When Manner of Death is Accident or Homicide or Suicide then Time of Injury Indicator should be populated','Error'),

						--CONFIRM BELOW WITH AVINASH
						--('DimInjuryAtWorkId,InjuryNature,DimInjuryPlaceID,DimInjuryTransportID','InjuryAtWork= ''Y'' AND (InjuryNature IS NULL OR InjuryPlace IS NULL OR InjuryTransport IS NULL)', 'Injury occurred at work but InjuryNature Or InjuryPlace is missing', 'Error'),
						('Death.DimCertifierDesignId,DimInjuryAtWorkId','InjuryAtWork <> ''NULL''  AND Certifier <>''MEDICAL EXAMINER''','When Injury at Work is populated Certifier should be Medical Examiner','Error'),
						('DimInjuryAtWorkId,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND InjuryAtWork = ''NULL''','When Manner of Death is Accident or Homicide or Suicide then Injury at Work field should be populated','Error'),

						('InjuryNature,Death.DimCertifierDesignId','InjuryNature IS NOT NULL AND Certifier <>''MEDICAL EXAMINER''', 'When Injury Nature is populated Certifier should be Medical Examiner', 'Error'),
						('InjuryNature,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND InjuryNature IS NULL','When Manner of Death is Accident or Homicide or Suicide then Injury Nature should be populated','Error'),

						('DimInjuryPlaceId,Death.DimCertifierDesignId','InjuryPlace  <> ''NULL'' AND Certifier <>''MEDICAL EXAMINER'' ', 'When Injury Place is populated Certifier should be Medical Examiner', 'Error'),
						('DimInjuryPlaceId,Death.DimDeathMannerId','MannerOfDeath NOT IN (''Natural'',''Pending investigation'',''Could not be determined'',''Therapeutic complication'')  AND InjuryPlace = ''NULL''','When Manner of Death is Accident or Homicide or Suicide then Injury Place should be populated','Error'),

						('DimInjuryTransportId,Death.DimCertifierDesignId','InjuryTransport  <> ''NULL''AND Certifier <>''MEDICAL EXAMINER''', 'When Injury Transport is populated Certifier should be Medical Examiner', 'Error'),

						('DimInjuryTransportOtherId,DimInjuryTransportId','InjuryTransport <> ''OTHER'' AND InjuryTransportOther <>''NULL''', 'When Injury Transport Other field is populated then Injury Transport must have Other as value', 'Error'),
						('DimInjuryTransportOtherId,Death.DimCertifierDesignId','InjuryTransportOther <> ''NULL'' AND Certifier <>''MEDICAL EXAMINER''', 'When Injury Transport Other is populated Certifier should be Medical Examiner', 'Error')
