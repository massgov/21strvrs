...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
						...
						('FL_PRONOUNCEMENT_EXISTS,PRO_DATE,PRO_TIME','(FL_PRONOUNCEMENT_EXISTS = ''Y'' AND PRO_TIME IS  NULL ) OR (FL_PRONOUNCEMENT_EXISTS = ''Y'' AND PRO_DATE IS  NULL)', ' If FL_PRONOUNCEMENT_EXISTS is YES Pronouncement Date and Time should not be blank', 'Warning'),
						('PRO_DATE,PRO_TIME','( PRO_TIME IS NOT NULL AND PRO_DATE IS  NULL ) ', ' When Pronouncement Time is populated Pronouncement date cannot be blank', 'Error')
