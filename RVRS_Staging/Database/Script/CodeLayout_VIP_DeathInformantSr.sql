...
DECLARE @TargetTable VARCHAR(128) = 'DeathInformant',  -- replace this 
        @StandardFilter VARCHAR(1024)= '
					  ...
					  AND D.FL_VOIDED  = 0 
					  ',
	@TestFilter VARCHAR (MAX) = 'AND internal_case_number= 201940026364'    --test here:    @TestFilter VARCHAR (MAX) = ' AND internal_case_number= 201940053907'
...
DECLARE        
        ...
        @jiraID VARCHAR(16) = '169', @author VARCHAR(128) = 'Foyzur Rahman', -- replace author name and jira #
        @printflag bit = 1 , 
		@seq INT = 0 ,
		...
		@cfc int,@mfc int,
		@top VARCHAR(128) =  'TOP 1000 ',  -- For PROD leave this blank 
  ...
  DECLARE     @Workdb VARCHAR(128) = '[RVRS_testdb].',  -- For PROD leave this blank or set to @stagingdb
		@rc INT = 0,
	    @TargetCreate VARCHAR(MAX),@TargetColumns VARCHAR(MAX), @TargetColumnsMod VARCHAR(MAX),
		...
		@testflag bit = 1 -- set this to 0 for production 

    DECLARE	@targetloc varchar(128) =@Workdb --'[RVRS_PROD].[RVRS_ODS].' For PROD replace this by [RVRS_PROD].[RVRS_ODS]. 
...
('INFO_NME', 1,'','','FirstName','VARCHAR(128)', NULL, NULL),
('INFO_LST_NME',1,'','','LastName', 'VARCHAR(128)',NULL, NULL),
('INFO_MIDD_NME',1,'','','MiddleName', 'VARCHAR(128)',NULL, NULL),
('INFO_SUFFIX',1,'','','Suffix', 'VARCHAR(128)', NULL, '''NULL'''),
('INFO_RELATION_LIST',1,'','','RelationList', 'INT', NULL,NULL ),
('INFO_RELATION',1,'','','InformantRelation', 'VARCHAR(128)',NULL, '''NULL'''),
('INFO_RELATION_OTHER',1,'','','OtherInformantRelation','VARCHAR(128)',NULL, '''NULL''')
...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
                         ('FirstName','LEFT(FirstName,1) LIKE ''[^a-zA-Z]'' AND FirstName NOT LIKE ''-%''',   'Not a valid First Name',  'Error'),
						 ('MiddleName','LEFT(MiddleName,1) LIKE ''[^a-zA-Z]'' AND MiddleName NOT LIKE ''-%''',   'Not a valid Middle Name',  'Error'),
						 ('LastName','LEFT(LastName,1) LIKE ''[^a-zA-Z]'' AND LastName NOT LIKE ''-%''',   'Not a valid Last Name',  'Error'),
						 ('InformantRelation,OtherInformantRelation','OtherInformantRelation != ''NULL'' AND InformantRelation !=''OTHER'''  ,   'Not a valid InformantRelation',  'Error')
...
IF OBJECT_ID('tempdb..#dc') IS NOT NULL 
    drop table #dc
CREATE TABLE #dc (id INT identity(1,1), SourceColumn VARCHAR(128), TableName VARCHAR(128),  ColumName VARCHAR(128),Flag Bit, ObjectType VARCHAR(8) default 'Dim', DataType VARCHAR(128), DC Bit, OtherFilter VARCHAR(MAX), DCType VARCHAR(8))

insert into  #dc  (SourceColumn,TableName,ColumName, ObjectType,DataType, DC, OtherFilter, dcType) values
  ('Suffix','DimSuffix', 'DimSuffixId', 'Dim', 'INT',1, NULL, NULL),
  ('InformantRelation','DimInformantRelation', 'DimInformantRelationId', 'Dim', 'INT',1, 'f.InformantRelation = ''OTHER'' AND OtherInformantRelation = dc.', 'OtherOnly'),
  ('OtherInformantRelation','DimOtherInformantRelation', 'DimOtherInformantRelationId', 'Dim', 'INT',1, NULL, NULL),
  ('FirstName','Informant_FirstName' ,'FirstName','Fact', 'VARCHAR(128)',1, NULL, NULL),  
  ('LastName','Informant_LastName','LastName', 'Fact', 'VARCHAR(128)',1, NULL, NULL), 
  ('MiddleName','Informant_MiddleName', 'MiddleName', 'Fact', 'VARCHAR(128)',1, NULL, NULL) 
...
*****************************************************************************************
DELETE FROM '+@Workdb+'[RVRS].[DeathOriginal] WHERE Entity = '''+ @TargetTable+'''
TRUNCATE TABLE '+@Workdb+'[RVRS].[' +@TargetTable +']
...
IF OBJECT_ID(''' + @Workdb+ '[RVRS].['+ @TargetTable +'_Log]'') IS NULL 
	CREATE TABLE ' + @Workdb+ '[RVRS].['+ @TargetTable +'_Log] (Id BIGINT IDENTITY (1,1), SrId VARCHAR(64),' +IIF(@DcCreate IS NOT NULL,@DcCreate +',','')
					+IIF(@MoveCreate IS NOT NULL,@MoveCreate +',','') + @TargetCreate + ','  + @SourceCreate + ',' + 
...
'BEGIN TRY
    ...
    PRINT ' 
	ALTER TABLE #Tmp_HoldData_Final ADD ' + IIF (@DcCreate IS NOT NULL , @DcCreate + ',', '') + IIF (@DcFlagCreate IS NOT NULL, @DcFlagCreate +',','')+  IIF (@MoveCreate IS NOT NULL,@MoveCreate +',', '') +  @TableFlag +' BIT NOT NULL DEFAULT 0,LoadNote VARCHAR(MAX) 
	...
	WHILE (@cid <=  @cfc) 
	BEGIN 
			SELECT @SourceColumn = SourceColumn, @TableName = TableName, @ColumnName = ColumName, @ObjectType = ObjectType, @DataType = DataType,@DC= DC FROM #dc WHERE Id = @cid
  ...
  PRINT	'                '	+ @print + ''''  +   @recordNumberPrint + ''' +  CAST(@RecordCountDebug AS VARCHAR(10)) 
	...
	'INSERT INTO ' + @Workdb+ '[RVRS].['+ @TargetTable +'_Log]
			(
				...
				IIF (@DcColumns IS NOT NULL ,',' + @DcColumns,'') + '
				 ,' + @TargetColumns + '	
				 ,' + @SourceColumns + '
				'+ IIF (@MoveColumns IS NOT NULL,',' + @MoveColumns ,'') + '
				,SrCreatedDate
				,SrUpdatedDate
				,CreatedDate
				,' + @TableFlag + '
	...
	if @testflag = 1 
			print ' select * from '  + @TargetOriginal  
...
if @testflag = 1 
			print ' select * from '  + @Workdb  + '[RVRS].[Execution] '
