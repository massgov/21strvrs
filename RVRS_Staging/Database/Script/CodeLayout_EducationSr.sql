...
DECLARE @TargetTable VARCHAR(128) = 'Education',  -- replace this 
...
('DEDUC',1,'','','EducationId', 'VARCHAR(128)', NULL,NULL),
('AGE1_CALC',1,'','','AgeCalcYear', 'VARCHAR(128)', NULL,NULL)
...
INSERT INTO #validation (columnname , condition , errormessage,errortype) values 
				...
				('Person.AgeCalcYear,DimEducationId','(AgeCalcYear < 14 AND EducationId in (7,8,9)) OR (AgeCalcYear < 16 AND EducationId IN (8,9)) OR(AgeCalcYear < 5 AND EducationId IN (2,3,4,5,6,7,8,9))', ' Age cut off and Education Mismatch' , 'Warning')
...
insert into  #dc  (SourceColumn,TableName,ColumName,DimColumn, ObjectType,DataType, DC, OtherFilter, DCType,DCTableName,DimFilter) values --DCType = OtherOnly, StandardOnly, Standard&Other
	('EducationId','DimEducation', 'DimEducationId','SrcEducationId', 'Dim', 'INT',0, NULL, NULL,NULL,NULL)
...
'BEGIN TRY
	...
	BEGIN 
      ...
      IF @DC = 1
		BEGIN 
		PRINT 
			'/*THIS CODE IS TO GET MATCH FROM ' + isnull ( @dcTable,'')  + ' TABLE AND UPDATE THE ' + isnull( @ColumnName,'')  + ' WITH CORRECT VALUE,
						FOR THE RECORDS WHICH COULD NOT GET A MATCH IN ' + isnull(@TableName,'') + ' TABLE*/		
			'
